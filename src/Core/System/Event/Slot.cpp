...
namespace ece
{
	Slot::Slot(const Handle & handle): id(ece::EventManagerLocator::getService().getSlotID()), handle(handle)
	{
	}

	Slot::Slot(const Slot & copy): id(ece::EventManagerLocator::getService().getSlotID()), handle(copy.handle)
	{
	}

	Slot::Slot(Slot && move): id(ece::EventManagerLocator::getService().getSlotID()), handle(std::move(move.handle))
	{
	}
