...
static Instruction *previousinstruction (FuncState *fs) {
  static const Instruction invalidinstruction = static_cast<Instruction>(-1);
...
static void savelineinfo (FuncState *fs, Proto *f, int pc, int line) {
  ...
  luaM_growvector(fs->ls->L, f->lineinfo, pc, f->sizelineinfo, ls_byte,
                  MAX_INT, "opcodes");
  f->lineinfo[pc] = static_cast<ls_byte>(linedif);
...
void luaK_indexed (FuncState *fs, expdesc *t, expdesc *k) {
  ...
  t->u.ind.t = static_cast<lu_byte>(t->u.info);  /* register or upvalue index */
  if (t->k == VUPVAL) {
    t->u.ind.idx = static_cast<short>(k->u.info);  /* literal string */
    t->k = VINDEXUP;
  }
  else if (isKstr(fs, k)) {
    t->u.ind.idx = static_cast<short>(k->u.info);  /* literal string */
    ...
    t->u.ind.idx = static_cast<short>(cast_int(k->u.ival));  /* integer constant in proper range */
    t->k = VINDEXI;
  }
  else {
    t->u.ind.idx = static_cast<short>(luaK_exp2anyreg(fs, k));  /* register */
    t->k = VINDEXED;
  }
}
