...
LUA_API int lua_gc (lua_State *L, int what, ...) {
  ...
  switch (what) {
    ...
    case LUA_GCSETPAUSE: {
      ...
      setgcparam(g->gcpause, static_cast<lu_byte>(data));
      break;
    }
    case LUA_GCSETSTEPMUL: {
      int data = va_arg(argp, int);
      res = getgcparam(g->gcstepmul);
      setgcparam(g->gcstepmul, static_cast<lu_byte>(data));
      break;
    }
    ...
    case LUA_GCGEN: {
      ...
      if (minormul != 0)
        g->genminormul = static_cast<lu_byte>(minormul);
      if (majormul != 0)
        setgcparam(g->genmajormul, static_cast<lu_byte>(majormul));
    ...
    case LUA_GCINC: {
      ...
      if (pause != 0)
        setgcparam(g->gcpause, static_cast<lu_byte>(pause));
      if (stepmul != 0)
        setgcparam(g->gcstepmul, static_cast<lu_byte>(stepmul));
      if (stepsize != 0)
        g->gcstepsize = static_cast<lu_byte>(stepsize);
