...
namespace ece
{
	...
	inline void Camera::lookAt(const FloatVertex3u & target) { this->updatePosition(this->position, target); }

	inline void Camera::lookUpTo(const FloatVector3u & direction) { this->updatePosition(this->position, this->target + direction); }

	inline void Camera::moveTo(const Movable & object) { this->updatePosition(object.getPosition(), this->target); }

	inline void Camera::moveTo(const FloatVertex3u & position) { this->updatePosition(position, this->target); }

	inline void Camera::moveIn(const FloatVector3u & direction) { this->updatePosition(this->position + direction, this->target); }
