...
namespace ece
{
	namespace renderer
	{
		namespace opengl
		{
			...
			template <class T> static inline constexpr auto dataType() { throw std::runtime_error("This type cannot be passed."); }

			template<> inline auto dataType<short int>() { return DataType::SHORT; }
			template<> inline auto dataType<unsigned short int>() { return DataType::UNSIGNED_SHORT; }
			template<> inline auto dataType<int>() { return DataType::INT; }
			...
			inline FramebufferAttachmentChannel operator+(const FramebufferAttachmentChannel& left, unsigned short int shift)
			{
				return static_cast<FramebufferAttachmentChannel>(static_cast<unsigned short int>(left) + shift);
			}
