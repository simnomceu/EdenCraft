		...
		namespace rendering
		{
			...
			inline auto RenderContext::isCurrent() const noexcept
			{
				try {
					...
					ece::ERROR << "A RenderContext need to be managed by a std::shared_ptr, according to std::enabled_shared_from_this mother class specification. " << e.what() << ece::flushing;
					return false;
				}
			}
		} // namespace rendering
